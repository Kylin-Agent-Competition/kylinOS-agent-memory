...
def main() -> int:
    ...
    hits_gt = cli.search(collection, [1.0, 0.0, 0.0, 0.0], top_n=10, user_id=USER, now=NOW)
    print(f"[W3] top_n=10(>3) -> {len(hits_gt)} hits")
    assert len(hits_gt) == 3

    # W3-4: 超大 top_n -> 不崩，返回 3
    hits_huge = cli.search(collection, [1.0, 0.0, 0.0, 0.0], top_n=1000, user_id=USER, now=NOW)
