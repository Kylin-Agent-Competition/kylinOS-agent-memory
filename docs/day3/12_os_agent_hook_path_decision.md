...
> **决策：`NO_COMPLIANT_PRIMARY_FOUND / NO_APPROVED_BACKUP / BLOCKED_FOR_PRODUCTION`**
> D3-C 只确认 Pre-Chat、Post-Turn 和真实 Tool Result 的语义 seams。当前未找到可证明受支持、
> 且符合本模块“不修改官方 AI 助手源码”边界的生产接入点，也没有书面批准的备用路径。
> 必须保留阻断，不能自行切换为源码补丁、日志抓取、二进制注入或 Prompt 伪事件。
...
| 层级 | 决策 | 当前状态 |
|---|---|---|
| 生产主路径 | 只能使用官方明确支持的扩展点/订阅接口，并覆盖 Pre-Chat、Post-Turn 和真实 Tool Result 三个语义 seam | `NOT_FOUND / UNVERIFIED / BLOCKED` |
...
下列三个 seam 是契约需覆盖的语义位置，不是已选定的源码函数或修改授权。
生产路径必须先证明官方扩展点或事件订阅接口存在、在目标版本受支持，且无需修改官方源码。
未满足这些条件时，本文只保留契约和待验证位置，生产接入为 `BLOCKED`。
...
`SEMANTIC_CANDIDATE / PARTIAL`，不是已选主路径，也不是 `HOST_VERIFIED`。
...
`intentionrecognition` 线索，但未捕获结构化 Tool 事件。因此本文只保留“真实结果回调的语义
seam”，精确的合规扩展点仍为 `NOT_FOUND / BLOCKED / TD-007 / TD-009`。
...
| 可审查性 | 扩展接口说明、集成 diff、构建、安装、日志和回退可以逐项取证 |
| 风险 | 受上游版本、构建、Kaiming、KYSEC 和真实 Hook 可见性约束，必须经 D/L2 验证 |

这些标准用于后续审查具体扩展接口；本批次没有发现满足标准的接口，因此不设定生产主路径。
任何官方源码 patch 都超出 `os-agent-integration` 当前明示边界，需要新的范围和书面授权，不是 D3-C 默认退路。
...
- 可稳定关联 `tool_call_id` 和公共 `trace_id`；
...
| 未取得新授权就修改官方 AI 助手源码 | `PROHIBITED_IN_CURRENT_SCOPE` | 与本模块明示的“不修改官方 AI 助手源码”边界冲突 |
...
- 对外标识与能力差异；
- 不得宣称原生宿主事件或官方应用集成；
- 从备用恢复到届时已批准生产路径，或完整卸载的步骤。

## 7. 决策状态矩阵

| 路径 | 语义选择 | 实现 | 宿主证据 | 批准 | 当前可执行结论 |
|---|---|---|---|---|---|
| 官方受支持 Pre-Chat 扩展点 | 语义已定 | 未找到 | `NOT_OBSERVED / TD-008` | 后续接口需门禁 | 阻断 |
| 官方受支持 Post-Turn 扩展点 | 语义已定 | 未找到 | `PARTIAL / D2-C BLOCKED/E2` | 后续接口需门禁 | 阻断 |
| 官方受支持 Tool Result 扩展点 | 语义已定 | 未找到 | `NOT_VERIFIED / TD-007/009` | 后续接口需门禁 | 阻断 |
...
D3-C 已明确三个语义 seams 和合规接入标准，但当前**没有选出可执行的生产主路径**。
官方源码补丁与本模块边界冲突，已从主路径撤下；官方受支持扩展点尚未找到或验证。

当前也没有已批准备用路径。后续应提交具体官方接口证据，或提交独立 Qt 壳/Adapter 方案等待
D/E 书面决策；在此之前，正确动作是保持 `BLOCKED`，不是自行切换技术路线。
