...
- **权威层级声明（2026-09-03 增补）**：本文件保持 `CANDIDATE_FOR_FREEZE`。业务字段语义冲突时以 `KMA_UNIFIED_DATA_FORMAT_FREEZE_V1.md`（Canonical Business Schema v1，业务语义层 `FROZEN`）为准；本文件继续承载 Canonical 未覆盖的详细业务语义（七类业务概念、对象关系、字段处置、业务规则）。本声明**不改变**「十二、12.2 升级为团队冻结基线」的 8 项冻结条件。
...
| `captured_at` | FROZEN_BUSINESS_SEMANTIC | 捕获入库时间语义冻结；经 Canonical R-1 裁定为 Canonical 事件捕获时间字段；`collected_at` 为 legacy transport alias，业务语义一一对应，transport→business 必经 Adapter/Mapping；transport 层采纳/更名属 C/D 实现 handoff（TD-060） |
...
| 派生对象 | 处置 | 理由 / 说明 |
|----------|------|-------------|
| 公共事件字段 14 项 | DEFERRED（语义候选沿用） | 与 Day1 Schema 语义一致者沿用为候选；`collected_at` vs `captured_at` 已由 Canonical R-1 裁决（`captured_at` canonical / `collected_at` legacy transport alias）；transport 层采纳 `PENDING_D_CONFIRMATION`；宿主/协议证据 `PENDING_C_CONFIRMATION`/`PENDING_D_CONFIRMATION` |
...
| v1（修订1） | 2026-09-03 | 权威层级与语义裁决收口（day12-e-01）：头部增补「权威层级声明」（字段语义冲突以 Canonical v1 为准，不改变 §12.2 冻结条件）；§5.1 `captured_at` 行更新为 Canonical R-1 业务裁决（`collected_at` 为 legacy transport alias，C/D 实现 handoff 登记 TD-060）；§5.7 公共事件字段行同步 R-1 裁决；C-01 追加处置（Schema §2.5 过时描述已修正，HD-SCHEMA-14 终审保留）；C-03 当前选择改写为业务裁决并显式保留 C/D 实现 handoff。状态仍 `CANDIDATE_FOR_FREEZE`。 | E 轨道 |
