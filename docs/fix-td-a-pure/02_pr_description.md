# [A 轨] 技术债批量收口——11 项 TD Resolved + 1 项 Wontfix（REWORK 修订版）

> 分支：`fix/td-a-pure`（基于 main @ `5bd2c3e`，Day8 #38 已合并；**8 commits**：`18ae759`…`622d3c3`，已推送）
> Reviewer：D 主审；安全/降级/数据影响 E 补审
> 修订记录：2026-08-18 按 D 主审 REWORK 报告（R-1 必改 + #3-#8）修订；R-1 修正完成并回写台账/证据日志后复审

## 背景与目标

按技术债台账管理规则第 3 条（"代码合并 ≠ 技术债关闭，关闭需 Reviewer 确认验收标准达成"），将 A 轨**可关闭的 11 项技术债一次性收口**（含 3 项需麒麟 VM 实测的：TD-A-005-01/02/04），另 1 项经 VM 证实 SDK 无能力故标 Wontfix。剩余 3 项 Open（D6-EXEC-RACE 非阻断并发风险、D6-TOOL-PARTIAL / D7-CACHE-USER-DIMENSION 契约未定）本次不动，理由见"已知限制"。
...
- **`providers/embedding_provider.py`**：
  ...
  - TD-A-005-04：`model_info().name` 经 Bridge `get_default_model_name()` 动态查询 SDK 真实模型名（失败回退硬编码默认名）——**能力认证 SOURCE_VERIFIED + ABI_VERIFIED + PARTIAL（R-1 降级）**
...
### C++ Bridge（cpp-bridge/）
- **TD-A-005-04**：`EmbeddingBridge::get_default_model_name()`（dlsym `text_embedding_get_model_list` 等 4 符号）
  - **[Review #5 已修复]** 持 `mutex_` 保护 syms_/session_ 读取（与 embed 路径一致）
  - **[Review #4 已修复]** 注明 SDK 无 EmbeddingModelList 释放 API（进程级生命周期由 SDK 管理，不手动 free）；list[0]=默认模型假设明确标注待验证
...
- 不将 TD-A-005-04 升级 HOST_VERIFIED（R-1：证据非判别性；ABI 头保持 UNTESTED）

## 关联任务与技术债

- 本 PR 关闭：TD-A-005-02/03/04/05/06/07/08/09 + TD-A-D6-LLM-TOOL-INPUT + TD-A-D7-LLM-HANG-DEGRADE + TD-A-D8-CONTRACT-CATEGORY-SYNC（11 Resolved）+ TD-A-005-01（Wontfix）
- 维持 Open：TD-A-D6-EXEC-RACE / TD-A-D6-TOOL-PARTIAL / TD-A-D7-CACHE-USER-DIMENSION
- **[Review #2 登记]** TD-A-005-01/02 的 VM 探测（grep 无 cancel API + 4 线程并发挂起）无持久化日志——已在台账标注"宿主终端输出"，证据缺口登记；判别性补充（005-04 get_default_model_name 原始返回值）待宿主 VM 执行
...
| `docs/technical-debt/TECHNICAL_DEBT_REGISTER.md` | 11 Resolved + 1 Wontfix + R-1 降级 |
...
| `evidence/l2-kylin-vm/td_a_005_04_model_name.log` | R-1 非判别性说明 |
...
- TD-A-005-04（**非判别性，R-1**）：输出 model name 与硬编码回退名相同，未证明动态路径在宿主生效——已降级 PARTIAL，判别性证据（打印 get_default_model_name 原始返回值）待宿主补充
- TD-A-005-01/02：宿主终端探测（grep 无 cancel API；4 线程×100 并发挂起）——无持久化日志，证据缺口已登记（#2）
...
- TD-A-005-04：model_info() 首次调用多一次 SDK get_model_list 查询（毫秒级，仅状态查询）
...
- **TD-A-005-04 PARTIAL**：L2 证据非判别性（输出与硬编码回退名相同）；ABI 头保持 UNTESTED；`text_embedding_get_model_list` 基线标注原文档标禁调——判别性证据（打印 get_default_model_name 原始返回值）待宿主 VM 补充后可升级 HOST_VERIFIED
- **维持 Open（3 项）**：D6-EXEC-RACE（非阻断并发风险）、D6-TOOL-PARTIAL（partial result 无格式 + 契约字段）、D7-CACHE-USER-DIMENSION（契约无可信 user_id）
- **[Review #2]** TD-A-005-01/02 VM 探测无持久化日志（宿主终端输出）；005-04 判别性证据待补——三项待宿主 VM 执行后补齐证据缺口

## 回滚方式

- `git revert 622d3c3`（连同其下 7 commits）即恢复 main @ 5bd2c3e 基线
- 新增测试文件随之移除；生产代码回退后恢复原 TD 状态（Open），不影响既有功能
- C++ 改动回退后 `model_info()` 回退硬编码默认名（原行为，不破坏契约）
