...
| TD-018 | FRZ-IPC-004 客户端 deadline_ms 超时行为未实现 | memory-client/src/memory_client.cpp | Technical Debt | Medium | Open | C 轨成员 | D 主审 | D5 阶段 | 实现 deadline_ms + 100ms 客户端超时检查；超时后取消 pending 请求并 emit requestFailed(请求ID, TIMEOUT, "Deadline exceeded")；新增单元测试覆盖 deadline 到期、deadline+100ms 边界、deadline 未到期不触发；FROZEN_ALIGNED 仅覆盖协议帧结构与字段，不包含 FRZ-IPC-004 客户端超时行为 | PR #49 |
