...
| TD-A-005-04 | model_info.name 硬编码默认模型名 | memory-service/providers/embedding_provider.py, cpp-bridge | Technical Debt | Low | Resolved | A 轨成员 | D 主审 | 2026-08-10 | ✅ 已解决（SOURCE_VERIFIED + ABI_VERIFIED + HOST_VERIFIED，2026-08-18 判别性证据）：Bridge get_default_model_name() 用 void* 显式调用 SDK get_model_list（绕开不透明指针类型在 pybind 的 ABI 差异）+ 模型名缓存（SDK 重复查询 use-after-free，首次查询后缓存防二次调用段错误）；麒麟 VM 全量重编后 RAW 返回 ensemble-embd_gte-base_uint8-text、model_info().name 二次调用走缓存稳定（evidence/l2-kylin-vm/td_a_005_04_model_name.log 判别性结论）；回归测试 test_td_005_04_*（L1 FakeBridge 路径）；构建注意：vboxsf 时钟错误需 rm -rf build 全量重编 | PR #38 分支 fix/td-a-pure |
