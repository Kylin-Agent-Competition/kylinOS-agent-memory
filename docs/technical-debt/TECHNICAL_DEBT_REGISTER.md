...
| TD-A-005-01 | EmbeddingProvider 主动超时中断未实现（timeout_ms 无实际效果） | memory-service/providers/embedding_provider.py, cpp-bridge | Technical Debt | Medium | Wontfix | A 轨成员 | D 主审 | 2026-08-10 | ✅ 2026-08-16 麒麟 VM 确认：SDK 无 text_embedding_cancel/abort/interrupt API（grep /usr/include/kylin* 仅其他引擎 stopChat/stopRecognition 等）；现有缓解为最终方案——EmbeddingService 层 fut.result(timeout) 提供调用方超时保护（ERR_TIMEOUT 结构化返回不阻塞聊天线程），Bridge 内部无法主动中断 SDK 调用（无 API 可调） | PR #17 / PR #25 |
| TD-A-005-02 | embed_batch 并行策略未定（当前顺序调用） | memory-service/providers/embedding_provider.py | Technical Debt | Low | Resolved | A 轨成员 | D 主审 | 2026-08-10 | ✅ 2026-08-16 麒麟 VM 实测：4 线程 × 100 次并发 embed 挂起（timeout 30 无返回，SDK 单会话 unix socket 串行）；并发不安全 → 保持顺序调用为正确决策；L2 吞吐基线（Day9）亦证实单会话 ~14 req/s 为上限 | PR #17 |
...
| TD-A-005-04 | model_info.name 硬编码默认模型名 | memory-service/providers/embedding_provider.py, cpp-bridge | Technical Debt | Low | Resolved | A 轨成员 | D 主审 | 2026-08-10 | ✅ 已解决（能力认证：SOURCE_VERIFIED + ABI_VERIFIED + PARTIAL，D 主审 R-1 降级 2026-08-18）：Bridge 新增 get_default_model_name()（dlsym text_embedding_get_model_list 等 4 符号，nm 确认导出）；Provider model_info().name 动态查询真实模型名（失败回退硬编码默认）。L1 已证：FakeBridge 返回非空时 model_info().name=该值（test_td_005_04_real_model_name_from_bridge）+ 返回空时回退默认（test_td_005_04_fallback）。L2 局限：VM 输出 model name 与硬编码回退名相同（非判别性，未证明动态路径在宿主生效）；ABI 头保持 UNTESTED（embedding_abi_compat.h:34-35/68）；基线文档标注 text_embedding_get_model_list 原文档标禁调未运行验证。判别性证据（打印 get_default_model_name 原始返回值）待宿主 VM 补充后可升级 HOST_VERIFIED | PR #38 分支 fix/td-a-pure |
