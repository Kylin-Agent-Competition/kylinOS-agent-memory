...
| TD-A-005-04 | model_info.name 硬编码默认模型名 | memory-service/providers/embedding_provider.py, cpp-bridge | Technical Debt | Low | In Progress | A 轨成员 | D 主审 | 2026-08-10 | ⚠️ 动态查询路径宿主未生效（2026-08-18 判别性证据）：RAW bridge.get_default_model_name() -> ''（返回空串），model_info().name 实际走回退硬编码分支；L1 FakeBridge 路径已证（非空时生效/空时回退），但宿主 dlsym text_embedding_get_model_list 疑似未命中（nm 显示为 ai_runtime_core_text_embedding_service_call_* mangled 前缀，待核实裸符号）。待办：① 核实 .so 裸符号名并修正 dlsym 字符串；② VM 重验 RAW 返回值非空；③ 完成后再标 Resolved（SOURCE_VERIFIED+ABI_VERIFIED+PARTIAL 起步，判别后升级）。ABI 头保持 UNTESTED；基线标注 text_embedding_get_model_list 原文档标禁调 | PR #38 分支 fix/td-a-pure |
