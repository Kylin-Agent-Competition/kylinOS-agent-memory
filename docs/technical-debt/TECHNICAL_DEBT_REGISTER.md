...
| TD-004 | Vector Engine 原子索引代次/Collection 切换能力未验证 | Vector Provider / index rebuild | Risk | High | Open | gaoyizhe934 (B) | D | 2026-08-23 | 取得目标麒麟宿主原子切换及故障恢复 E4 证据，或由 D/B 冻结并验证 maintenance-window/routing-switch 等价方案；失败重建不得替换旧 serving generation，恢复路径须可审计。2026-08-23 V005 宿主实证：SDK 无 rename/swap/replace 原子切换 API，等价方案为 routing-switch（证据见 docs/day3/16_d5b_retrieval_vertical_slice_host_report_20260823.md + evidence/index.yaml B-D3-V005）；本 TD 保持 Open，待 D 轨确认冻结等价方案后关闭 | PR #20 / PR #51 |
...
| TD-017 | KnowledgeCandidate 六类结构化字段未获 E Domain 无损承载 | memory-service/domain/knowledge.py, memory-service/service/candidate_governance.py, memory-service/providers/extraction_provider.py | Technical Debt | Medium | Open | E 轨成员 | D 主审 | 2026-09-10 | PR #47 Reviewer（Medium）指出：A 轨 KnowledgeCandidate v0.2（extraction_provider.py L167-200）含六类结构化字段共 13 个 Optional[str]——conditions/evidence（通用+证据）、steps/expected_result（workflow）、problem/outcome/reproducible（case）、template_body/parameters（template）、priority（constraint）、failure_reason/avoid_condition/alternative（failure_experience）；当前 Day5 E Domain（domain/knowledge.py）仅冻结 knowledge_type/source_event_id/content_summary/confidence_score 等核心字段，_build_knowledge（candidate_governance.py L329-358）明确注释"六类结构化字段为抽取侧细节，不映射进 Domain"，上述 13 个字段尚无 E Domain 正式无损承载契约（content_ref DEFERRED 待 D，D3 §5.3 Knowledge 字段表无结构化字段）；不得宣称已通过 content_summary（仅承载 fact 文本）或 primary_category（开放分类标签）完整保存；关闭条件：① D/E 冻结正式结构化承载契约（字段级映射或 content_ref 存储形态）；② 为 failure_experience/workflow/case/template 等类别补充无损映射测试（结构化字段进出 Domain 一致性）；③ 本 TD 仅登记 Medium 结构化承载缺口，不把 PR #47 中任何 High 门禁旁路问题降级为技术债，High 仍须由代码修复关闭 | PR #47 |
