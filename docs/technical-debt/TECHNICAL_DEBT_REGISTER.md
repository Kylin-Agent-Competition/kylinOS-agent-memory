...
| TD-A-005-09 | EmbeddingService 启动期 SDK 缺失无降级（server 构造直接抛 RuntimeError） | memory-service/embedding/embedding_service.py, memory-service/embedding/server.py, memory-service/providers/embedding_provider.py | Technical Debt | Medium | In Progress | A 轨成员 | D 主审；安全/降级影响 E 补审 | 2026-08-14 | ✅ ① 完成：server.py 已有 provider 注入点（TD-A-005-09 修复路径注释）；EmbeddingService 构造时 EmbeddingProvider() 失败（SDK 缺失）→ 兜底 _SdkMissingProvider（embed 抛 ERR_SDK_NOT_LOADED → ok+degraded 空向量；health bridge_loaded=false；server 不再构造即崩）；回归测试 test_td_a_005_09.py（3 项）；L1 270 passed + 49 skipped；⏳ ③ 麒麟 VM "so 缺失"端到端证据待执行（宿主：sudo mv .so 后启动 server 验证 embed/health） | PR #17 / Day5 PR |
