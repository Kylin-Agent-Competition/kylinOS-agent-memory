...
| TD-A-005-04 | model_info.name 硬编码默认模型名 | memory-service/providers/embedding_provider.py, cpp-bridge | Technical Debt | Low | Resolved | A 轨成员 | D 主审 | 2026-08-10 | ✅ 已解决并 VM 验证：Bridge 新增 get_default_model_name()（dlsym text_embedding_get_model_list / embedding_model_list_get_count / embedding_model_list_get_model / embedding_model_info_get_model_name），Provider model_info().name 动态查询真实模型名（失败回退硬编码默认）；麒麟 VM 实测输出 model name: ensemble-embd_gte-base_uint8-text / dim=768 / loaded=True（evidence/l2-kylin-vm/td_a_005_04_model_name.log）；回归测试 test_td_a_005_03_05.py::test_td_005_04_*；L1 267 passed + 47 skipped | PR #38 分支 fix/td-a-pure |
