...
| 测试日期 | 2026-07-29 | `day2_smoke_run.log` |
...
| TC-6 | **任意文本传参** | 任意字符串 | ⚠️ dim=768, L2=1.000000（`text_embedding()` **不验证文本内容**） | HOST_VERIFIED / E4 | 同上 |
...
2. **纯空白字符 = 空输入** — 纯空白字符的向量前5维与 Day 1 空输入前5维一致（`-0.0726 0.0508 -0.0465 0.0409 0.0271`），SDK 将空白视为空输入。
3. **超长文本不截断** — 2170 bytes 中文重复文本正常返回 768 维向量，`text_embedding()` 在 2170 bytes 以内不截断（实测）。实际长度上限需进一步测试。
4. **`text_embedding()` 不验证文本内容** — 任意字符串（含"错误模型名"等非法内容）均被 SDK 正常向量化。错误模型名验证属于 `text_embedding_init_model()` 的职责，与 `text_embedding()` 无关。
