...
- 基线：`origin/main@0820036`（2026-09-03 同步，已含 D11D PR #115、C-D11 PR #116、D11B PR #111、D10D PR #120 等）。
...
| 5 | 同 VM 端到端验收执行与证据归档：在统一麒麟 VM、同一 Commit 上执行五类业务验收与主演示；输出业务验收报告、原始日志/结果，SHA-256 登记 `evidence/index.yaml` | 工作项 1–4；A/B/C/D 跨轨输入就绪 | 报告与原始证据一致、可复跑命令、D 主审材料 | 待开始（无 VM 证据前为 `UNVERIFIED`） |
...
| 7 | 收口与审查：整理变更、验收证据、`UNVERIFIED` 项与跨轨依赖，交由 D 轨非作者 Reviewer 审查；Review 意见回填本清单与 PR | 工作项 1–6；Review 可用性 | 审查材料完整、Review 结论回填 | 待开始 |
...
| 统一 VM/systemd/trace/DB 与遗忘持久化 | D（D11D PR #115、D10D PR #120 已合并） | 消费统一环境与持久化结果，不代实现 |
...
- D11D（PR #115）已合并并含麒麟 VM L2 证据与 `packaging/systemd` 产物；C-D11（PR #116）5 步主演示编排已合并；D11B（PR #111）、D10D（PR #120）已合并；`main` 现为 `0820036`。
...
- 基线：`origin/main@0820036`（含 D11D PR #115、C-D11 PR #116、D11B PR #111、D10D PR #120 等）。
...
1. 基线与环境盘点（`origin/main@0820036`、D11D 统一 VM/交付、C-D11 主演示编排、E 轨业务实现与测试、冻结契约与验收样例）。
