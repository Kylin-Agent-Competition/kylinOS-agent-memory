# D6-D 多源事件持久化契约规划（草案 v0.5）

- **编制日期**：2026-08-31（v0.5 按 Review #83 第五轮修订）
- **编制人**：opencode（D 轨开发 Agent）
- **状态**：DRAFT v0.5 — D 已确认决策点并重冻结，按 Review #83 Reviewer E 第五轮 REWORK 修订；待 Reviewer E 再签（ADR-013/014 流程）
...
> **v0.5 第五轮修订（Review #83 Reviewer E 第五轮，HIGH-01）**：① handler 固定顺序重排——`payload 结构预检 → trusted identity precheck（先于任何 idempotency_cache lookup，cache-bypass 防护）→ EventPipeline 纯计算（仅一次）→ privacy-safe request_fingerprint（sensitive/security reject/consent reject 内容身份取固定安全占位 `<SENSITIVE-OMITTED>`）→ UoW.execute_idempotent 单事务（business_fn：event identity compare → consent → admission → dedup → source_event 落库 → response cache）`（D-1/D-7/D-9/D-11 修订）；② request_fingerprint 计算时机前移到 Pipeline 纯计算之后（敏感判定真源唯一 = Pipeline，不复制第二套逻辑）；③ 高敏低熵内容**不得经 `idempotency_cache._request_fingerprint` 旁路落盘**——正文/摘要/内容指纹/请求指纹四路皆不持久化敏感派生值（HIGH-01/D-12 扩展）；④ cache replay 语义：允许纯计算 pipeline，禁止重复业务副作用（HIGH-01）；⑤ trusted identity mismatch 在幂等查找前 fail-close（cache-bypass），新增对应 L1 用例（第八节）。
...
- 契约文档（本文档 v0.5 + ADR-013 + ADR-014 + 冻结文档回写）
...
- **编排**（v0.5 / HIGH-01 统一固定顺序）：payload 结构预检（schema_version 显式预检）→ **trusted identity precheck（先于任何 idempotency_cache lookup，cache-bypass 防护）** → `EventPipeline.process(raw)` **纯计算（仅一次，无 DB 副作用，是 request_fingerprint 的唯一敏感判定真源）** → **基于 PipelineResult 计算 privacy-safe `request_fingerprint`**（普通事件用 canonical `event_content_identity`；**sensitive/security reject/consent reject 事件内容身份取固定安全占位 `<SENSITIVE-OMITTED>`，不由敏感正文派生确定性 hash**）→ **`UoW.execute_idempotent` 单事务进入（HIGH-02）**；`business_fn` 内：**event_id 冲突处置**（按 `user_id+event_id` 点查既有行：immutable identity 一致=幂等重放返回首次完整结果+`duplicate_reason='idempotent_replay'`；不一致=`EventIdentityConflict`→INVALID_REQUEST，HIGH-01；跨用户 IntegrityError fail-close，MEDIUM-03）→ **D 轨 consent 前置判定**（`consent_scope=none` → REJECT，handler 层实现，不改 `security/`）→ `SourceAdmissionPolicy.evaluate(result, ctx_svc)`（复用不改，`ctx_svc` 由 `RequestContext → ServiceRequestContext` 显式转换构造）→ `source_events` 落库（`processing_status='pending'`；**敏感/security/consent reject 事件 `content_fingerprint` NULL**，HIGH-03；`find_dedup_group_head`+insert 同事务，MEDIUM-04；独立事务=**不与 outbox 同事务**、但与幂等缓存同 UoW，HIGH-02）→ response cache 同事务写入
...
- **敏感/未授权事件的时间与内容派生值**：`source_events.content_fingerprint` NULL（HIGH-03/D-12）且 **`request_fingerprint` 对敏感内容取固定安全占位 `<SENSITIVE-OMITTED>`（v0.5/HIGH-01）**——正文 / `content_summary` / `content_fingerprint` / `request_fingerprint` 四路均不持久化可由敏感正文派生的确定性 SHA-256；`idempotency_cache` 只落占位指纹
...
| D-7 | 事件入口 handler | **本版包含 handler** ✅：`event.ingest` IPC 写方法（ADR-014 路由扩展，production 默认不注册）+ gateway handler 编排（**v5**：payload 结构预检 → trusted identity precheck → Pipeline 纯计算（仅一次）→ privacy-safe request_fingerprint → event_id 冲突处置 → consent 前置 → admission → 落库 → response cache，单事务） | 仅 repository 层 | 用户指示本版自建 handler，不等 D6-C；路由新增走 ADR-014 冻结流程 |
...
| D-11 | 请求级幂等冲突语义与事务边界（第四轮 HIGH-02 / 第五轮 HIGH-01 计算时机） | **复用既有 `idempotency_cache` + `request_fingerprint`** ✅ v4 新增：`request_fingerprint = sha256(method + 业务语义字段[e.v event_id/occurred_at 规范化/content identity/...])`（不含 trace_id/request_id/deadline_ms/schema_version/idempotency_key）；`same triple + same fingerprint → cache replay`；`same triple + different fingerprint → IdempotencyConflictError → INVALID_REQUEST`；**事务边界**：幂等检查 + event collision + pipeline/admission + source_event 写入 + response cache 全部在 `UoW.execute_idempotent` 单事务`内完成（不拆两事务）；**v5 修订（HIGH-01）**：request_fingerprint **在 Pipeline 纯计算之后基于 PipelineResult 计算**（敏感判定真源唯一 = Pipeline）；sensitive/security reject/consent reject 事件内容身份取固定安全占位 `<SENSITIVE-OMITTED>`，**不由敏感正文派生确定性 hash**——`idempotency_cache._request_fingerprint` 不落敏感派生值；cache replay 允许复用纯计算 pipeline、禁止重复业务副作用；trusted identity 校验先于幂等缓存查找 | 拆成幂等与业务两个事务（否决） | FRZ-IPC-005「相同幂等请求只产生一次副作用」依赖同事务 + 进程级单写锁（uow.py 已实现）；idem_1 先后用于 evt_A/evt_B → 冲突拒绝（第四轮 HIGH-02）；pre-pipeline 提前算指纹会复制第二套 sensitive/normalization 真源或把敏感低熵 hash 泄入幂等缓存（第五轮 HIGH-01） |
| D-12 | 敏感/未授权事件内容指纹（第四轮 HIGH-03 / 第五轮 HIGH-01 扩展） | **`content_fingerprint` 持久化 NULL** ✅ v4 新增：`is_sensitive_matched=true` / `sensitivity=high/critical` / security reject / consent reject / `consent_scope=none` 事件落库时 `content_fingerprint` 必须 NULL，**不参与内容级指纹去重/分组**（dedup_group/duplicate_of 为空）；未来敏感判重须走 keyed HMAC / 隐私保护型指纹 + 独立 ADR；**v5 扩展（HIGH-01）**：敏感/未授权事件 `request_fingerprint` 内容身份取固定安全占位 `<SENSITIVE-OMITTED>`——**`idempotency_cache` 亦不落由敏感正文派生的确定性 hash**，正文/摘要/内容指纹/请求指纹四路皆不持久化敏感派生值 | 持久化普通确定性 SHA-256（否决） | 低熵敏感值（手机号/身份证/口令）可离线枚举 SHA-256，不等价不可恢复脱敏（Review #83 第四轮 HIGH-03）；幂等缓存旁路为同源风险（第五轮 HIGH-01） |
...
| 场景 | 行为 | 映射 |
|---|---|---|
| 事件已存在（同 event_id，immutable identity 一致） | **幂等重放**：返回首次持久化完整结果（source_event_id + 既有 admission_decision/reason_code + duplicate=true + duplicate_reason=idempotent_replay）；**允许复用已执行的一次纯计算 pipeline（无副作用），不重跑 event collision/consent/admission/落库等业务副作用（HIGH-01 v5）** | 正常路径，非错误 |
| 事件已存在（同 event_id，immutable identity 不一致） | 抛 `EventIdentityConflict` → INVALID_REQUEST（HIGH-01） | INVALID_REQUEST（不回显标识） |
| 请求级幂等冲突（同三元组 + 不同 request_fingerprint） | 抛 `IdempotencyConflictError` → INVALID_REQUEST（HIGH-02，如 idem_1 先 evt_A 后 evt_B）；同三元组 + 同指纹 → cache replay 返回首次 response（纯计算 pipeline 可复用，业务副作用不再执行，HIGH-01 v5） | INVALID_REQUEST（不回显标识） |
...
1. **用户隔离**：所有查询强制 `user_id` 过滤（Repository 层，[02 §16.6]）+ `UNIQUE(event_id)` 全局唯一 + immutable identity 一致性（同 event_id + identity 不一致 → 拒绝，D-10）+ 跨用户 event_id IntegrityError fail-close（不回读他人事件，MEDIUM-03）；**payload 身份仅属事件声明（test/validation 下为自证一致性，非可信身份认证）**，production ACTIVE 前须以独立 trusted host identity + fail-close 比对为硬门禁（D-9 / MEDIUM-02）；**trusted identity 校验必须先于任何 idempotency_cache lookup（v0.5/HIGH-01，cache-bypass 防护）**——cache replay 不可携带与缓存归属不同的 user 身份绕过隔离；
2. **原文隔离**：正文/敏感载荷不落库、不入日志；只落脱敏摘要 + 受控引用；**敏感/安全类事件 content_summary/raw_payload_ref 强制 NULL**（is_sensitive_matched=true / high-critical / security-consent reject / consent_scope=none），普通质量型 AUDIT_ONLY 保留已脱敏摘要（BLOCKER 回归项 + MEDIUM-07）；**敏感/未授权事件 `content_fingerprint` 持久化 NULL**（HIGH-03 / D-12），不参与内容级指纹去重；**request_fingerprint 对敏感内容取固定安全占位（v0.5/HIGH-01），正文/摘要/内容指纹/请求指纹四路皆不持久化敏感派生 SHA-256（含 `idempotency_cache._request_fingerprint` 旁路）**；
...
| **请求级幂等：同三元组 + 同 request_fingerprint → cache replay（返回首次 response，无新增业务副作用；允许复用一次纯计算 pipeline，不重复 event collision/admission/落库）** | request fingerprint（HIGH-02 / D-11 + v5 HIGH-01 replay 语义） |
...
| **高敏 request_fingerprint（v5）：content_summary 含高敏（sk_xxx）初投 sensitivity=none → ① Pipeline 判 sensitive 升级；② request_fingerprint 内容身份取固定安全占位 `<SENSITIVE-OMITTED>`；③ idempotency_cache._request_fingerprint 不含可由敏感正文直接派生的确定性 hash；④ content_fingerprint 落库 NULL** | 敏感指纹旁路防护（HIGH-01 v5 / D-11 / D-12） |
| **trusted identity cache-bypass（v5）：production ACTIVE profile 下 trusted identity 与 payload.user_id mismatch → 在 idempotency_cache lookup 前 fail-close → INVALID_REQUEST；某三元组已有 cache response 也不放行** | 身份时序（HIGH-01 v5 / D-9） |
...
2. Reviewer E（谢嘉然）**再次签署**（Review #83 已 REWORK，v5 修订后重提）
...
- [x] D-7：本版包含 handler（`event.ingest`，ADR-014；**v5 固定顺序**）
- [x] D-8：consent_scope=none 由 D 轨 handler 前置 REJECT（不依赖 E 轨）
- [x] D-9：身份可信源 = 宿主注入；**v3：payload 身份仅属事件声明，非独立可信身份源；ACTIVE 前须独立 trusted host identity + fail-close 比对（MEDIUM-02）**；**v4：content_summary provenance 门禁（MEDIUM-05）**；**v5：trusted identity 校验先于 idempotency_cache lookup（cache-bypass 防护，HIGH-01）**
- [x] D-10：immutable identity 组成（v4，HIGH-01）
- [x] D-11：请求级幂等 request_fingerprint + 单事务边界（v4，HIGH-02）；**v5：privacy-safe 指纹 + Pipeline 后置计算 + cache replay 仅纯计算（HIGH-01）**
- [x] D-12：敏感/未授权事件 content_fingerprint NULL（v4，HIGH-03）；**v5：request_fingerprint 固定安全占位，idempotency_cache 不落敏感派生值（HIGH-01）**
...
3. **Reviewer E 复核 v5 修订**：trusted identity 前置（HIGH-01）、Pipeline 后置计算 request_fingerprint（HIGH-01）、敏感指纹安全占位（HIGH-01）、cache replay 纯计算语义（HIGH-01）、以及前四轮已关闭项（identity/事务/敏感指纹 NULL/pending 谓词/跨用户 fail-close/dedup 原子/content_summary 门禁/TD-D6D-001）。

**后续流程**：ADR-013（source_events 表设计）+ ADR-014（event.ingest 路由）→ D 决策确认 v5 + Reviewer E 再签 → 任务卡定稿 → 代码实现。
