...
| [ADR-013：新增 `source_events` 表持久化多源事件](013-source-events-table.md) | ✅ D 已决策（2026-08-31，v4 按 Review #83 第四轮重冻结），待 Reviewer E 签署 | 方案 A：FRZ-DB-001 新增第 6 张表（35 字段 + 5 索引）；event_id 全局唯一 + identity collision 契约（v4：独立稳定 identity 字段集，排除 idempotency_key/session_id，纳入 actor_id）；指纹"保留事件+标记去重"（dedup_group 含 user scope，dedup head 与插入同 UoW 原子）；敏感命中 content_summary/content_fingerprint 强制 NULL；processing_status 首次 pending + 三条件消费谓词 |
| [ADR-014：新增 `event.ingest` IPC 写方法](014-event-ingest-method.md) | ✅ D 已决策（2026-08-31，v4 按 Review #83 第四轮重冻结），待 Reviewer E 签署 | 方案 A：FRZ-IPC-007 新增写方法；flat payload；schema_version IPC required override；consent 前置 REJECT；Context Adapter 与宿主注入身份（payload 身份仅属声明）；duplicate_reason 区分；request_fingerprint 幂等冲突 + UoW.execute_idempotent 单事务；CANDIDATE/BLOCKED_BY_HOST_MAPPING |
