...
- **幂等（唯一真源 + 冲突语义）**：
  - **envelope 为权威源**。`idempotency_key` 权威值 = IPC envelope 顶级字段（FRZ-IPC-006，写操作建议提供）→ 未提供则取 `payload.metadata.idempotency_key` → 两者同提供且不一致 → `INVALID_REQUEST`。
  ...
  - **幂等冲突**：相同三元组 + **不同请求语义/payload** → `INVALID_REQUEST`（防不同事件误复用同一 key 被静默吞掉）。判定用请求指纹：`sha256(规范化 method + 语义关键字段)`，规范化字段 = envelope 权威值（user_id/session_id/trace_id）+ host_turn_id + source_reference + is_final + finalized_at；指纹**内嵌缓存 response JSON**（不改 DDL）。
...
- **落库语义（turn 新建/更新）**：
  ...
  - **失败语义**：resolver 解析失败且 turn 不存在（INSERT 场景）→ `INTERNAL_ERROR`（safe）；**禁止编造正文、禁止以空串替代**（`turns.original_user_text` NOT NULL 冻结语义）；
