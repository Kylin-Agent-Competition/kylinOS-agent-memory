...
- **状态**：提议（D3-B 冻结候选，待 D 主审；涉及评测时 E 补审）
- **日期**：2026-08-03
...
两路 Provider 分别返回 1 起始排名，Memory Service 在完成用户/版本/安全
硬过滤和去重后执行 RRF。
...
1. Provider 响应结构校验；
2. `rank > 0` 校验；
3. 同一通道同一 `memory_id` 只保留最佳 rank；
4. 按 `memory_id + version_id` 回源 SQLite；
5. 用户、当前版本、状态、有效期、敏感度、冲突和遗忘硬过滤；
6. 按 `memory_id` 聚合。
...
- D4 需要建立 RRF 纯函数、重复命中去重、确定性排序和单路降级测试；
...
“提议”改为“已采纳”必须经过任务卡要求的 D 主审，涉及用户隔离、安全和
正式评测口径的部分还需 E 补审。
