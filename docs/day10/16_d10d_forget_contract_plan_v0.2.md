# D10-D 精准遗忘持久化契约规划（草案 v0.2）

- **编制日期**：2026-09-01（v0.2 按 Reviewer E 冻结回复修订）
- **编制人**：opencode（D 轨开发 Agent）
- **状态**：DRAFT v0.2 — 已按 Reviewer E 冻结回复（`APPROVED_WITH_STAGED_RUNTIME`，2026-09-01，发布于 PR #98 评论区）修订业务与安全语义；**仍待 ADR-015/016 立项 + D 决策 + Reviewer E 签署**后转正式契约
- **对照基线**：main @ `d4f564c`（PR #84 day11 联调合并后）；`docs/architecture/D3_MEMORY_BUSINESS_CONTRACT_V1.md`（§5.5 ForgetPlan，`CANDIDATE_FOR_FREEZE`）；`deliverables/D4_DB_INITIAL_DESIGN_FREEZE_20260817.md`（FRZ-DB-001）；`deliverables/D4_IPC_PROTOCOL_FORMAL_FREEZE_20260817.md`（FRZ-IPC-007）；`docs/adr/013-source-events-table.md`、`014-event-ingest-method.md`（D6-D 契约 v5，编号占用先例）；PR #94（D10-E B 轨代行，selector 边界/状态机前置）；PR #82（D10-B Vector 删除工作计划）
...
本规划 v0.2 全量吸收该回复；**本文件本身不替物理 Schema / ADR / Runtime 实现作完成性背书**。

---

## 一、背景与目标（台账 R55 / D10-D）

D6-D（R35）已规划多源事件持久化（`source_events`，ADR-013/014 已签署合并）。D10-D 承接「精准遗忘与删除一致性」的 **D 轨持久化职责**：
...
### 禁止修改（红线）
- 不修改冻结 FRZ-IPC-001~007 既有字段/错误码/envelope（forget 方法走 ADR-016 新增）
...
### 3.1 复用字段（ForgetPlan，D3 §5.5）+ v0.2 冻结修订
...
| `affected_count` | Optional[int] | **禁止模型生成**（execute 结果）；进入 `awaiting_confirmation` 前必须满足 `resolved_target_ids is not None` 且 `affected_count == len(resolved_target_ids)` |
...
CREATE TABLE forget_plan (
    ...
    target_selector       TEXT    NOT NULL,          -- 用户输入选择器
    ...
    selection_hash        TEXT,                      -- Preview/Selection 稳定 Hash（确认凭据绑定 + 审计引用，非正文）
    ...
    delete_mode           TEXT    NOT NULL DEFAULT 'soft', -- soft / hard（v0.2 冻结语义；hard 物理清除 Runtime fail-closed）
    ...
    affected_count        INTEGER,                   -- execute 结果，禁止模型生成
    executed_at           TEXT,
...
- 版本链：`001_initial_schema → 20260826_add_trace_id → 20260831_preference_versions → 20260831_add_source_events（D6-D）→ 20260901_add_forget_plan`（以实现时 `alembic heads` 实际链为准，避免多 head）
...
- **硬删除（`hard_delete=true`）**：不可恢复的物理正文清除；完成后在本产品承诺覆盖的 SQLite / FTS5 / Vector / Cache / 普通日志 / 导出及可重建来源中不得保留可检索明文残留。**不得实现为「只置 is_deleted=1 后声称彻底删除」**；关键数据层无法物理清除时**必须 fail-closed，不得自动降级为软删后返回「硬删除成功」**。
...
## 五、确认凭据与 preview/execute 分离（v0.2 修订）

1. **preview**：规则引擎解析 `ForgetPlan` → 生成 `resolved_target_ids` + `affected_count` + `selection_hash`（Preview/Selection 稳定 Hash）→ 生成**确认凭据**（随机 32B，**只存 SHA-256 哈希**，不存明文）→ 返回 `{preview_result, selection_hash, credential_ttl}`；凭据绑定 `user_id + forget_plan_id + selection_hash`。
2. **execute**：携带 `forget_plan_id + 确认凭据` → 校验绑定（用户/计划/目标快照一致）+ 未过期 + 未使用（防重放）→ 仅消费已确认的 `resolved_target_ids` 执行软删事务 → 标记凭据已消费 → 审计写入；**不得在 execute 阶段重新扩大目标集合**。
...
| F-6 | IPC 方法形态 | `forget.preview` / `forget.execute` 两个写方法（ADR-016） | preview/execute 分离是 `[02 §10.1]` 红线，必须两方法 |
...
| **审计零正文验收**：带唯一 Sentinel 的合成敏感正文执行遗忘 → 扫描 forget_audit 全部字段（含 target_selector/reason/details/source_reference/error_message）+ 服务日志 + 导出 → Sentinel 0 命中 | 安全 |
...
- 软删后检索不复现目标正文；审计表零正文（Sentinel 扫描）
...
1. **本文件（契约规划 v0.2）入库** ← 本 PR：吸收 Reviewer E 冻结回复（APPROVED_WITH_STAGED_RUNTIME），作为 ADR-015/016 前置
