...
- **日期**：2026-09-01
- **状态**：Plan（待 orchestrator 审查 + 用户确认后进入 Phase 2 / Build）
...
| §三.2 | 确认凭据（一次性，绑定 user+plan+hash+TTL+防重放）；幂等复用 FRZ-IPC-005（不新建表）；`delete_mode` 可信来源由 ADR-016 冻结（MEDIUM-04） | S3/S4 + ADR-016 §4.2/§4.7；request_fingerprint 敏感占位（对齐 ADR-014 v5） | 落地（D3/D4 待裁） |
...
| §四.4 | 软删优先（is_deleted=1，FTS 不再命中）；硬删物理清除 Runtime fail-closed；软删可见性以 SQLite 为准 | S5 dispatcher（knowledge/preference 权威状态；event fail-closed D2） | 落地（D2 待裁） |
...
| §四.9 | delete_mode 门禁：ADR-016 冻结可信来源；Repository 不推导；LLM 不终判；hard 接线前 fail-closed | ADR-016 §4.2 + S5 fail-closed | 落地（D3 待裁） |
| §五.1 | preview：规则引擎生成 resolved/affected/selection_hash → 凭据（32B，只存 SHA-256）→ 返回 {preview_result, selection_hash, credential_ttl}；Preview 后清 selector | S3/S4 + S9（resolver seam，D6） | 落地（D6 待裁） |
...
| §六 | Outbox 高优先级：方案 A（nullable priority + 部分索引 + worker ORDER BY priority DESC, next_retry_at ASC + CHECK 扩展 'forget'） | S1/S6 | 落地（D1 待裁） |
...
## 四、待 D 决策点（Plan 批准前需裁决，Build 前闭合）

| # | 决策点 | 推荐 | 影响范围 |
|---|--------|------|---------|
| D1 | outbox `aggregate_type` CHECK 值域扩展（SQLite 重建） | 按契约 §六 方案 A 扩展（重建迁移，保留数据）；备选 = 不改 CHECK 仅 priority 驱动（弱化） | 迁移 + outbox/worker + 入队 |
| D2 | event 目标（source_events）软删机制 | 推荐 fail-closed + 登记 TD-E（staged）；不推荐本任务内改 source_events（超出红线） | S5 dispatcher + executed_count 语义 |
| D3 | `delete_mode` 可信输入来源冻结 | ADR-016 冻结为「可信宿主显式提供，默认 soft」；接线前 hard fail-closed | ADR-016 + S7 |
| D4 | Preview 凭据明文回传信道 | 响应回传一次明文（服务端只存哈希） | ADR-016 + handler |
...
- Build 将严格限定在任务卡（18_）批准范围内：schema/迁移/Repository/UoW/outbox 优先级/软删主路径/fail-closed/测试 +（ADR-016 签署后）gateway seam。
- 红线不随 Phase 2 解除：不修改冻结契约与既有实现、不接 Vector 清理、不实现 hard/cascade/full_reset/time_window/回滚完整逻辑、不 push/不创建 PR、WSL 结果不作宿主证据。
