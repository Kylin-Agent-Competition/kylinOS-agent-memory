...
- 每次请求入口（含合并等待路径）调用 `recover_hung_bridge_executor()`：若任一 in-flight 超过 `_embed_hang_threshold_ms`（默认 60s，远大于单次超时 5s）仍未完成 → 判定永久挂死 → 重建 executor 释放挂死 worker。
- 重建后 in-flight 清空、`_embed_hang_recovered` 计数递增；health 新增 `executor` 分项暴露可观测性。
...
- 不把 WSL/沙箱结果当宿主证据；L2 验证项如实标注"待 VM 运行"。
