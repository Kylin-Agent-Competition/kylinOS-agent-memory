...
P2-B = IN_PROGRESS / BLOCKED_UNTIL_5_OF_5_E2E（R4–R9 已落地：V2 binding、sealed source → per-sample clones、provenance、dispatch 重接线、receipt provenance；R10 真实 retrieval 载体待 D/E 确认）
...
### 9.9 FTS-channel Forget 5/5 record（2026-09-07，PARTIAL NON-FORMAL）

执行 commit `d60dbe6e5d4e4c9583fb1eca250f811a8d10f75c` 上，V2 sealed source
binding 的五个 Forget sample 已通过 FTS-only 真实 preview → execute →
Outbox Worker / Router / deletion-consumer ACK → FTS realtime → full FTS
rebuild → receipt 链路。五条记录的 missed / wrongly deleted / cross-user
violation / realtime residual / rebuild residual 均为 0。详细身份、SHA、回归与
CI 证据见
[32_d13d_phase2_forget_e2e_20260907.md](32_d13d_phase2_forget_e2e_20260907.md)。

该结果仅是 FTS-channel `5/5 / PARTIAL NON-FORMAL` 记录。按 `29_` / `30_` 的
双通道 Gate，P2-B 仍为 `BLOCKED_PENDING_VECTOR_DUAL_CHANNEL`；本记录未注入
真实 Vector provider，未验证 Vector pre-delete 正向命中、Vector 删除、Vector
realtime residual、Vector full rebuild/requery 或 per-channel Vector
provenance，因此不宣称完整 Forget 5/5 closure。
