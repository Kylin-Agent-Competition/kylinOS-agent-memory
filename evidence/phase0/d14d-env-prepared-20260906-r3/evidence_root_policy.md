...
5. 历史 evidence root 永不改写（含早期 ENV_PREPARED root）。
```
...
- 本 ACTIVE ENV_PREPARED evidence root：
  `evidence/phase0/d14d-env-prepared-20260906-r3/`
- 历史 root（SUPERSEDED，仅供审计、不作为下游输入）：
  `evidence/phase0/d14d-env-prepared-20260906/`（r1）、
  `evidence/phase0/d14d-env-prepared-20260906-r2/`（r2）
- D13D I3d / A package 消费入口（本 ACTIVE root）：
  `clean_state_gate.sh`（固定 fail-closed Gate 脚本）+
  `raw/`（含正向 PASS 与受控负向 FAIL 的可复核运行证据）、
  `snapshot_identity.json`、`environment.json`、`dependency_identity.json`。
