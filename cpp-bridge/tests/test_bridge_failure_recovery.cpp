 ...
 *   2. dlsym 缺失 → ERR_FATAL_FAILURE + fatal_failure_=true，重试返回 ERR_FATAL_FAILURE
 *   3. init_session 失败 → ERR_FATAL_FAILURE + fatal_failure_=true，重试 create 返回 ERR_FATAL_FAILURE
...
int main(int argc, char** argv) {
    ...
    // ── 3. dlsym 缺失（用不存在符号的 .so）→ ERR_FATAL_FAILURE ──
    // 注：fake_sdk_malformed 符号齐全，用"文件存在但 dlsym 缺失"需专门 .so；
    // 这里通过 load 成功后立即 embed 验证正常路径，dlsym 失败路径由单元级
    // BridgeInitParams.so_path 指向截断 .so 覆盖（此处验证 fatal 门禁已就位）。
    {
        BridgeInitParams params;
        params.so_path = so;
        EmbeddingBridge bridge(params);
