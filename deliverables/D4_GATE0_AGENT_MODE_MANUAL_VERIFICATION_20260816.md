...
| 检查项 | 操作 | 预期 | 结果 |
|--------|------|------|:----:|
| V5-1 工具失败 | 断网/无效参数触发工具失败 | 失败状态建模，不产生成功结果 | ⬜ 待补测 |
...
3. **取消场景已验证**（V5-2：`User stop task mode!`）；**失败场景已观测**（V5-3 补测：`ls -la /root` → `权限不够`，宿主日志无沉淀记录），但端到端「失败不沉淀」仍需 Hook/Memory Service 集成后验证（详见 `evidence/l2-kylin-vm/d4_agent_mode_5_0_3_20260816/v5_3_fail_no_precipitate_evidence.md`）。
