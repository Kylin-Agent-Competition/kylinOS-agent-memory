...
| FRZ-IPC-007 | 方法路由表（活跃 3 项：echo / health / memory.retrieve；ADR-010 `turn.finalized`、ADR-014 `event.ingest`、ADR-019 `forget.preview` / `forget.execute` 标 CANDIDATE / BLOCKED_BY_HOST_MAPPING；ADR-018 `knowledge.detail` / `conflict.compare` / `lifecycle.status` 标 CANDIDATE / BLOCKED_BY_TRUSTED_IDENTITY_AND_D8D_PERSISTENCE；memory.store 未实现返回 UNSUPPORTED_METHOD；evidence.record 已按 P0-4 移除） | 2026-08-07（2026-08-17 更正；2026-08-27 ADR-010；2026-08-31 ADR-014；2026-09-02 ADR-018/019 扩展） | ✅ 协议冻结（已签署生效；ADR-010/014/018/019 均完成 D 决策 + Reviewer E 签署） |
...
> **2026-09-02 扩展（ADR-018 批准，PR #122 HEAD `c6ecbae`，Reviewer E APPROVED）**：FRZ-IPC-007 新增只读候选方法 `knowledge.detail` / `conflict.compare` / `lifecycle.status`，统一标 **`CANDIDATE / BLOCKED_BY_TRUSTED_IDENTITY_AND_D8D_PERSISTENCE`**；production 默认不注册 → `UNSUPPORTED_METHOD`。权威 user identity 仅取 `RequestContext.user_id`；cross-user/unknown/stale/legacy 使用 scoped empty。`knowledge.detail` 对 `evidence_unmapped` 允许受控只读并显式返回 `evidence_tier=null`，但不得进入 lifecycle/conflict 自动裁决。relation/evidence 与 conflict 使用 strict limit、stable exclusive cursor，并由真实 `gateway.protocol.encode()` 对完整 envelope 执行 65536-byte UTF-8 JSON body budget；不得静默截断、漏项或重复。完整 payload、投影、分页和激活门禁见 `docs/adr/018-knowledge-conflict-lifecycle-read-ipc.md`。本扩展不构成 Runtime、L1/L2 或麒麟宿主验证证据。
