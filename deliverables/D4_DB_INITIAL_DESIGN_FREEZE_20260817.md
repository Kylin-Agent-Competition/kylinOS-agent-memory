...
> ✅ **本声明已正式生效（Reviewer E 谢嘉然 2026-08-20 签署）**：R-5/R-6/R-7 已由 D 决策采纳（ADR-005/006/007，2026-08-17，方案 A；Reviewer 指派 E），冻结文档 §2.2.5/§4.1 已回写；冻结人（周子腾，D）签署 + Reviewer E（谢嘉然）签署完成，转为正式冻结。文档基线 Commit `b29c6b8`（仅指本声明关联的文档基线，**非代码基线**——自研库实现零启动，见 §二.3）。
...
> 状态说明：✅ = 内容定稿（含已采纳 ADR 支撑项，已签署正式生效）；⏳ = **条件冻结**（依赖项未批准，不得据此写 DDL）。

| 编号 | 冻结对象 | 定义位置 | 状态 | 依赖 |
|------|---------|---------|:---:|------|
| FRZ-DB-001 | 核心表 5 张 + 4 项索引 + FTS5 `memory_fts`，以及经 ADR 批准的兼容扩展 | 冻结文档 §2.2-2.4；需求 v1.3 §二 FR-DB-001 | ✅ | R-6 已采纳（ADR-006，复合 PK）；ADR-011/013/015/017 扩展已采纳 |
...
> **2026-09-02 扩展（ADR-015 批准，D 已决策 + Reviewer E 已签署，PR #112 APPROVED）**：FRZ-DB-001 新增 `forget_plan` / `forget_audit` 两张精准遗忘表；`forget_audit` 坚持最小审计零正文。`outbox` 仅新增 nullable `priority INTEGER DEFAULT 0`、部分索引 `idx_outbox_priority`，并将 `aggregate_type` CHECK 兼容扩展为 `('turn','memory','forget')`；优先级 `0`=普通、`1`=删除类、`2`=预留 urgent。既有表/列/索引/触发器/FTS5 定义不变；迁移 `20260901_add_forget_plan.py` 必须保留数据地对称重建 outbox 并维持 Alembic 单 head。确认凭据只存 SHA-256，selector 在 Preview 后清除，`executed_count != affected_count` 不得进入 `completed`。详见 `docs/adr/015-forget-persistence.md`。本扩展仅为设计/契约正式冻结，对外能力仍为 **`PARTIAL / staged implementation`**，不构成 Runtime 或麒麟宿主验证证据。

> **2026-09-02 扩展（ADR-017 批准，PR #122 HEAD `c6ecbae`，Reviewer E APPROVED）**：FRZ-DB-001 新增 `memory_relation` / `memory_conflict` / `memory_conflict_member` / `memory_lifecycle_receipt` 四表，并为 `memory_entries` additive 增加 `knowledge_id`、独立 CAS `row_revision`、`knowledge_type`、`conditions` 与生命周期真源字段。身份冻结为 `memory_id=canonical decimal memory_entries.id`、`version_id=v{memory_entries.version}`；`version` 保持内容/索引版本，所有 optimistic writer 必须迁移为 `row_revision` CAS，遗漏任一 writer 即 Gate fail。Knowledge ingress 必须消费同用户 `source_events.admission_decision` 与 E 轨对象类型约束；新 Knowledge 与唯一 primary evidence relation 同 UoW 原子写入。legacy 不猜测，关系/冲突/回执零自由文本旁路，Lifecycle mutation 与必要 Outbox 同事务。完整 DDL、CHECK、索引、幂等与 downgrade 门禁见 `docs/adr/017-relation-conflict-lifecycle-persistence.md` 及批准草案 `docs/day8/07_d8d_adr017_018_draft.md`。本扩展只完成 canonical 设计冻结，不构成 Runtime、Migration、L1/L2 或麒麟宿主证据。
...
| R-5 | **DB 对外错误码与 envelope 域** | 以 IPC 冻结契约为准（5 枚举 + `status/data/server_ts`）；`ERR_*`/`ok/result/error` 按 ALIGN-002/003 冻结后对齐；错误码变更走 IPC 冻结 ADR（FRZ-IPC-002） | ✅ 已采纳（ADR-005，D 决策方案 A；Reviewer E 已签） |
| R-6 | **idempotency_cache 主键** | 复合 PK `(user_id, session_id, idempotency_key)`；DDL 按此实现 | ✅ 已采纳（ADR-006，D 决策方案 A；Reviewer E 已签；冻结 §2.2.5 已回写） |
| R-7 | **迁移基线命名** | 基线 `001_initial_schema.py`，后续 `YYYYMMDD_<desc>.py` | ✅ 已采纳（ADR-007，D 决策方案 A；Reviewer E 已签；冻结 §4.1 已回写） |
...
| 项 | 决策 | 状态 |
|----|------|------|
| R-5 | ADR-005 方案 A：DB 层对外按 IPC 冻结契约（5 枚举 + status/data/server_ts） | ✅ 已采纳（D 决策 2026-08-17；Reviewer E 已签） |
| R-6 | ADR-006 方案 A：复合 PK (user_id, session_id, idempotency_key) | ✅ 已采纳（D 决策 2026-08-17；Reviewer E 已签；冻结 §2.2.5 已回写） |
| R-7 | ADR-007 方案 A：基线 001_initial_schema.py + 后续 YYYYMMDD_<desc>.py | ✅ 已采纳（D 决策 2026-08-17；Reviewer E 已签；冻结 §4.1 已回写） |

> 三项阻断项已解除：D4-D 可据需求 v1.3 与 ADR-005/006/007 直接实现（migrations 基线、复合主键 DDL、错误码/envelope 映射）；Reviewer E（谢嘉然）已于 2026-08-20 签署，正式生效。
...
> 本声明已由冻结人确认 + Reviewer E（谢嘉然，2026-08-20）签署，转为正式冻结（R-5/6/7 已采纳，阻断已解除）。
