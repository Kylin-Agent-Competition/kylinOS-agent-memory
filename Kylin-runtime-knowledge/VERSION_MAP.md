...
> **用途**: **版本对照索引 / Agent 速查入口**——Agent 编码时快速定位各仓库源码、版本与 VM 运行环境的对应关系
> **更新时间**: 2026-09-01（实测复核 VM 已安装版本；基线值源 2026-08-16 环境基线 v2 + memorymap 能力边界定案）
> **目标环境**: 银河麒麟 V11 2603 x86_64 (VirtualBox，VM `Kylin-desktop-neo`)

> ⚠️ **本文件为「版本对照索引 / Agent 速查入口」**，用于 Agent 快速定位版本、源码和接口关系；**VM 实际版本、ABI 与宿主行为仍以对应环境基线（`docs/baseline/`）及 L2/L3 evidence 为最终事实依据**。
> **版本对照表已更新到新基线 v2**（2026-08-16 实测）。旧基线值见 `docs/baseline/v2-20260816/02_*` 的「旧基线」列。
> 源码 Commit 与 VM 安装二进制的精确构建映射**未经证明的条目一律标记「待复核」**，不因版本号一致即视为已证明。
...
| 3 | kylin-coreai-embedding / libkylin-coreai-embedding | **1.2.0.0-0k0.4** | **1.2.0.0-0k0.4** | 1.2.0.0 | `openkylin/nile-sp2` | 63aed6f3 | ⚠️ 待复核（头文件来源 Commit `63aed6f3`；VM 0k0.4 与该源码 Commit 的精确构建映射待复核） |
...
| 6 | kylin-ai-vector-engine | **1.2.0.1-0k1.0** | **1.2.0.1-0k1.0** | 1.2.0.1 | `openkylin/huanghe` | b95cf43 | ⚠️ 待复核（VM 版本已实测 1.2.0.1-0k1.0；与该源码 Commit `b95cf43` 的精确构建映射待复核） |
...
> 🧪 **VM 已安装版本实测复核（2026-09-01）**：上表 #1~#9 及 §三「VM 已安装版本」列已通过麒麟 VM（SSH 127.0.0.1:2222 · `kylin-agent@kylin-agent-pc`）`dpkg -l`/`dpkg-query -W` 实测复核，结果与 2026-08-16 基线 v2 一致；`kylin-aiassistant`=5.0.3、`kylin-ai-memorymap`=2.0.23 经 kaiming 应用目录（`/sysroot/var/opt/kaiming/bin/*` → `binary/5.0.3` / `binary/2.0.23`）确认。证据命令输出见 PR #92 处置回复。**源码 Commit 与安装二进制的精确构建映射仍未证明，保持「待复核」口径**。
...
| # | 包名 | VM 版本 | 说明 |
|---|---|---|---|
| 1 | kylin-ai-abstract-models | 1.2.0.1-0k1.0 | 内部 GitLab: `gitlab2.kylin.com/kylinos-src/kylin-ai-abstract-models.git` |
| 2 | kylin-gte-base-model | 1.0.0.1-0k0.9 | Gitee 空仓库，仅有模型文件包 |
| 3 | kytensor-client | 2.49.0.6-ok7k0.14 | 无公开仓库 |
| 4 | kytensor-server | 2.49.0.6-ok7k0.14 | 无公开仓库 |
| 5 | kytensor-python | 2.49.0.6-ok7k0.14 | 无公开仓库 |
| 6 | onnxruntime-backend | 1.0.1-ok6k0.1 | Gitee 空仓库 |
| 7 | kytensor-llm | 2.0.0-ok15k0.11 | 新增，ggml 本地 LLM 推理 |
| 8 | llm-backend | 1.0.1-ok8k0.2 | 新增，kytensor llamacpp backend |
| 9 | gf-arise-llama-ponn | 02.00.08-1 | 新增，本地 LLM 推理 |
| 10 | kylin-ai-knowledge-base-service | 1.2.0.0-0k1.0 | 新增，官方知识库服务 |
| 11 | kylin-ai-document-qa-service | 1.2.0.0-0k1.0 | 新增，文档问答服务 |
| 12 | kylin-ai-document-service | 1.2.0.0-0k0.6 | 新增，文档服务 |
| 13 | kylin-ai-recollect-service | 1.0.0.0-0k1.0 | 新增；**已定案：memorymap 记忆内核（截图+OCR+CLIP+Milvus+SQLite，06 报告 §2.4）** |
| 14 | kyai-data-management-service | 1.2.0.0-0k1.10 | 新增，数据管理业务服务 |
| 15 | kylin-ai-parser-extension | 1.2.0.0-0k0.4 | 新增 |
| 16 | kylin-ai-python-env | 1.2.0.3-0k0.2 | 新增 |
| 17 | ai-kylin-qwen-plus-cloud-model | 1.0.0.0-0k0.2 | 新增，云模型 |
...
| `kylin-coreai-embedding/` | openkylin/kylin-coreai-embedding | openkylin/nile-sp2 | Embedding SDK C API 头文件（来源 Commit `63aed6f3`；与 VM 0k0.4 二进制的精确构建映射待复核） |
...
1. **匹配级仓库**（✅）：已确认版本/接口基线对应，可作为实现参考；**最终 ABI 与运行行为仍以 VM 实测为准**（「版本对应」不等于「Git commit 与安装二进制构建来源已精确证明」）
